package com.ravensoft.daniel.Throtletest;
 ...
 */
public class ThrotleView extends SurfaceView implements SurfaceHolder.Callback, View.OnTouchListener
{
    ...
    public ThrotleView(Context context)
    {
        super(context);
    ...
    public ThrotleView(Context context, AttributeSet attributes, int style)
    {
    ...
    public ThrotleView(Context context, AttributeSet attributes)
    {
    ...
    private void drawJoystick(float newX, float newY)
    {
        if(getHolder().getSurface().isValid())
        {
            ...
            float hypotenuse = newY - centerY;//(float) Math.sqrt(Math.pow(newX - centerX, 2) + Math.pow(newY - centerY, 2));
            float sin = 1;//(newY - centerY) / hypotenuse; //sin = o/h
            float cos = 0;//(newX - centerX) / hypotenuse; //cos = a/h
...
//                myCanvas.drawCircle(newX, newY, hatRadius - (float) i * (ratio) / 2 , colors); //Draw the shading for the hat
                myCanvas.drawRect(newX-160,newY-50, newX + 160 , newY + 50 , colors);
            }
...
//                }
                ...
                if(displacement < baseRadius)
                {
                    ...
                    joystickCallback.onJoystickMoved(0, -1 * (e.getY() - centerY)/baseRadius, getId());
                }
                else
                {
                    float ratio = baseRadius / displacement;
                    float constrainedX = centerX ;//+ (centerX - centerX) * ratio;
                    float constrainedY = centerY + (e.getY() - centerY) * ratio;
                    drawJoystick(constrainedX, constrainedY);
                    joystickCallback.onJoystickMoved((constrainedX-centerX)/baseRadius, -1*(constrainedY-centerY)/baseRadius, getId());
                }
            }
            else
                drawJoystick(centerX, centerY);
